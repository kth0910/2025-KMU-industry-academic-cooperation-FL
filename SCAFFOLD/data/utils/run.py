...
_CURRENT_DIR = Path(__file__).parent.resolve()   # == …/data/utils
BASE_ROOT    = _CURRENT_DIR.parent / "datasets"  # == …/data/datasets   ★
import sys
import os
...
def main(args):
    BASE_DIR      = BASE_ROOT / f"emnist_sim{args.sim}" / args.dataset  # ★
    _DATASET_ROOT = BASE_DIR                 # torchvision 이 받는 download 경로
    _PICKLES_DIR  = BASE_DIR / "pickles"
    BASE_DIR.mkdir(parents=True, exist_ok=True)
    _PICKLES_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for subset_id, client_id in enumerate(
        ...
        with open(_PICKLES_DIR / f"{subset_id}.pkl", "wb") as f:
    ...
    if args.type == "user":
        ...
        with open(Path(args.root) / "all_stats.json", "w") as f:
...
def generate_all_similarity_versions(base_args):
    for sim, alpha in [(0, 0.0), (10, 0.5), (100, 10.0)]:
        print(f"\n=== Generating EMNIST dataset with similarity={sim} ===")
        args             = argparse.Namespace(**vars(base_args))   # deep-copy
        args.sim         = sim                                    # ★ 새 필드
        args.alpha       = alpha
        args.dataset     = "emnist"
        args.root        = str(BASE_ROOT / f"emnist_sim{sim}")    # ★
        main(args)
        args_dict = vars(args)
        args_dict = {k: str(v) if isinstance(v, Path) else v for k, v in args_dict.items()}

        
        with open((BASE_ROOT / f"emnist_sim{sim}" / args.dataset / "args.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sim", type=int, default=0,
                    help="(내부용) similarity flag 0/10/100")
    args = parser.parse_args()
    generate_all_similarity_versions(args)
